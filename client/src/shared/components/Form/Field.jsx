...
const generateSelect = () => {
  const FieldComponent = ({ className, label, tip, error, name, onChange2,  ...otherProps }) => {
    const fieldId = uniqueId('form-field-');
    return (
      <StyledField
        className={className}
        ...
        data-testid={name ? `form-field:${name}` : 'form-field'}
      >
        {label && <FieldLabel htmlFor={fieldId}>{label}{onChange2.toString()}</FieldLabel>}
        <Select id={fieldId} invalid={!!error} name={name} onChange2={onChange2}  {...otherProps} />
        {tip && <FieldTip>{tip}</FieldTip>}
        {error && <FieldError>{error}</FieldError>}
      </StyledField>
    );
